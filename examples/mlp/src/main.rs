...
use inferox_core::{Backend, TensorBuilder, Tensor, Model};
...
fn load_model_from_binary(path: &str) -> Result<Box<dyn Model<Backend = CandleBackend, Input = CandleTensor, Output = CandleTensor>>, Box<dyn std::error::Error>> {
    unsafe {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <model-library-1.dylib> [model-library-2.dylib] ...", args[0]);
        eprintln!("\nExample (from workspace root):");
        eprintln!("  {} target/release/libmlp_classifier.dylib target/release/libmlp_small.dylib", args[0]);
    ...
    for (name, _) in engine.list_models() {
        ...
        let input = backend.tensor_builder().from_vec(input_data, &[1, input_size])?;
