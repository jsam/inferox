...
use crate::{
    cache::ChunkCache, config::XetConfig, hf_api::HfApiClient, xet::XetProtocolClient, CacheStats,
    FileInfo, RepoInfo, Result,
};
...
impl HfXetClient {
    ...
    pub async fn download_file(
        ...
        if file_info.lfs.is_some() {
            self.download_lfs_file(repo_id, filename, revision, &output_path)
                .await?;
        } else {
            let bytes = self
                .hf_api
                .download_file(repo_id, filename, revision)
                .await?;
...
mod tests {
    ...
    fn test_matches_patterns() {
        assert!(HfXetClient::matches_patterns(
            "model.safetensors",
            &["*.safetensors"]
        ));
        assert!(HfXetClient::matches_patterns(
